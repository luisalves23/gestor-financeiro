...
### 18/06/2026 | 09:30
* **Etapa:** Funcionalidade de Liquidação de Contas (Passo 18.2 e 18.3 de 18.3) - Motor JS de Pagamento.
* **O que foi feito:** Substituí a renderização fixa por uma condicional que injeta o botão `✔` apenas em contas não pagas. Separei a lógica de cliques das tabelas em controladores independentes e criei a inteligência de atualizar o objeto, alterar o status para `pago`, atualizar o `localStorage` e engatilhar o recálculo da UI.
* **Commit Realizado:** feat: implementa mutacao interativa de status e liquidacao de contas
* **Raciocínio:** O fluxo de "Contas a Pagar" precisava ser finalizável para não acumular distorções no painel. Em vez de deletarmos a conta após o pagamento (o que apagaria o histórico do usuário), criamos uma mutação de estado. Ao clicar no check, a conta é etiquetada como `pago`. Instantaneamente, a função `calculateDailyTarget` isenta essa conta da "Meta de Hoje" e a desconta do "Saldo em Conta" global, replicando o comportamento real de uma carteira de pagamentos fluida e protegendo a integridade dos dados passados.
